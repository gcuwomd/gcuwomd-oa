...
<template>
  <div class="container">
    ...
    <p>
      ...
      但本应用将以高度的勤勉、审慎义务对待这些信息。除本隐私权政策另有规定外，在未征得您事先许可的情况下，本应用不会将这些信息对外披露或向第三方提供。本应用会不时更新本隐私权政策。
      您在同意本应用服务使用协议之时，即视为您已经同意本隐私权政策全部内容。本隐私权政策属于本应用服务使用协议不可分割的一部分。
    </p>
    <n-ol class="primary-list">
      <n-li>适用范围</n-li>
      <n-ol style="list-style: lower-alpha">
        <n-li>在您注册本应用帐号时，您根据本应用要求提供的个人注册信息；</n-li>
        <li>
          在您使用本应用网络服务，或访问本应用平台网页时，本应用自动接收并记录的您的浏览器和计算机上的信息，包括但不限于您的IP地址、浏览器的类型、使用的语言、访问日期和时间、软硬件特征信息及您需求的网页记录等数据；
        </li>
      </n-ol>
      <n-li>您了解并同意，以下信息不适用本隐私权政策：</n-li>
      <n-ol>
        <n-li>您在使用本应用平台提供的搜索服务时输入的关键字信息；</n-li>
        <n-li
          >本应用收集到的您在本应用发布的有关信息数据，包括但不限于参与活动、成交信息及评价详情；</n-li
        >
        <n-li>违反法律规定或违反本应用规则行为及本应用已对您采取的措施。</n-li>
      </n-ol>
      <n-li>信息使用</n-li>
      <n-ol>
        <n-li>
      ...
      <n-ol>
        ...
        <n-li>
          如您出现违反所在国家有关法律、法规或者本应用服务协议或相关规则的情况，需要向第三方披露；
        </n-li>
        <n-li>
        ...
        <n-li> 其它本应用根据法律、法规或者网站政策认为合适的披露。 </n-li>
      </n-ol>
      <n-li>
      ...
      <n-ol>
        ...
        <n-li> 通过本应用所设cookies所取得的有关信息，将适用本政策。 </n-li>
      </n-ol>
      <n-li> 信息安全 </n-li>
      <n-ol>
        <n-li>
      ...
      <n-ol>
        ...
        <n-li>
          网信委保留随时修改本政策的权利，因此请经常查看。如对本政策作出重大更改，网信委会通过网站通知的形式告知。
        </n-li>
      </n-ol>
    </n-ol>
    <p>
